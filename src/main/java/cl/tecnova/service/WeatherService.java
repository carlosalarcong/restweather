...
import cl.tecnova.WeatherAppProperties;
...
public class WeatherService {
    ...
    private static final String BY_CITY_URL = "http://api.openweathermap.org/data/2.5/weather?q={city}&APPID={key}";

    private static final String BY_METRIC_URL = "http://api.openweathermap.org/data/2.5/weather?q={city}&units={metric}&APPID={key}";
    ...
    @Cacheable("city")
    public Weather getCity(String city) {
        logger.info("Requesting current weather for {}", city);
        URI url = new UriTemplate(BY_CITY_URL).expand(city, this.apiKey);
        return invoke(url, Weather.class);
    }

    @Cacheable("metric")
    public Weather getMetric(String city, String metric) {
        logger.info("Requesting current weather for {}/{}", city, metric);
        URI url = new UriTemplate(BY_METRIC_URL).expand(city, metric, this.apiKey);
        return invoke(url, Weather.class);
    }
